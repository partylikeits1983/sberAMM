...
describe("SberAMM Unit Tests", () => {
    ...
    // it("Second PID (admin): Withdraw and WithdrawFees", async () => {
        // expect(await SberAMM.viewEarnedFees(secondPID, USDT.address)).to.be.eq(mediumAmount.mul(3).div(1000)); // .mul(3).div(1000) == 0.003
        // expect(await SberAMM.viewEarnedFees(secondPID, USDC.address)).to.be.eq(0);
        // await SberAMM.withdrawFees(secondPID)
        // expect(await SberAMM.viewEarnedFees(secondPID, USDT.address)).to.be.eq(0);
        // expect(await SberAMM.viewEarnedFees(secondPID, USDC.address)).to.be.eq(0);
        // await expect(SberAMM.withdrawFees(secondPID)).to.be.revertedWith("Zero fees");

        // const withdr = await SberAMM.withdrawPreview(secondPID)
        ...
        // await expect(SberAMM.withdraw(secondPID)).to.be.revertedWith("No pool shares to withdraw");
        // await expect(SberAMM.withdrawPreview(secondPID)).to.be.revertedWith("No pool shares to withdraw");
        // await expect(SberAMM.viewEarnedFees(secondPID, USDC.address)).to.be.revertedWith("No shares found for the user");
    // });
/*
    it("Not stable swap", async () => {
        console.log("Swap 50,000 USDT to ETH in liquidity pool with 100,000 USDT and 50 ETH")
        let ethPriceBeforeSwap = ethers.utils.formatEther(await SberAMM.exchangeRate(secondPID, ETH.address))
        console.log(`1 ETH = ${ethPriceBeforeSwap.substring(0, 4)} USDT`)

        // const balanceA_t0 = await USDT.balanceOf(user0.address);
        const balanceC_t0 = await ETH.balanceOf(user0.address);
        ...
        const balanceC_t1 = await ETH.balanceOf(user0.address);
        // console.log("Balance A after swap %s", balanceA_t1)
        // console.log("Balance C after swap %s", balanceC_t1)

        let ethPriceAfterSwap = ethers.utils.formatEther(await SberAMM.exchangeRate(secondPID, ETH.address))
    ...
    it("Not stable swap", async () => {
        ...
        await SberAMM.connect(user1).swap(secondPID, ETH.address, swapAmount);
    ...
    it("WithdrawFees from stable", async () => {
        ...
        const previewParamsFeeC = await SberAMM.viewEarnedFees(secondPID, ETH.address)

        const USDTbeforeFee = ethers.utils.formatUnits(await USDT.balanceOf(admin.address))
        const ETHbeforeFee = ethers.utils.formatUnits(await ETH.balanceOf(admin.address))
        await SberAMM.withdrawFees(secondPID)
        const USDTafterFee = ethers.utils.formatUnits(await USDT.balanceOf(admin.address))
        const ETHafterFee = ethers.utils.formatUnits(await ETH.balanceOf(admin.address))

        const deltaAFee = (Number(USDTafterFee) - (Number(USDTbeforeFee))).toFixed(1)
        const deltaCFee = (Number(ETHafterFee) - (Number(ETHbeforeFee))).toFixed(1)
    ...
    it("Should Check All Possible Reverts", async () => {
        ...
        await expect(SberAMM.createPair(USDT.address, ETH.address, pairFee, true)).to.be.revertedWith("Pair already exists");
        ...
        await expect(SberAMM.swap(firstPID, ETH.address, mediumAmount)).to.be.revertedWith("Address: call to non-contract");
        ...
        await expect(SberAMM.viewEarnedFees(secondPID, ETH.address)).to.be.revertedWith("No shares found for the user");
